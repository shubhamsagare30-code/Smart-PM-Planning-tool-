    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">Assign resources to projects with capacity tracking</p>
        </div>
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
            {allocations.map((a) => (
              <tr key={a.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                ...
                <td className={`px-4 py-3 font-semibold ${getUtilColor(a.allocation_percentage)}`}>{a.allocation_percentage}%</td>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div><label className="label">Resource</label>
            <select className="input" required value={form.resource_id} onChange={(e) => setForm({ ...form, resource_id: +e.target.value })}>
              {resources.map((r) => <option key={r.id} value={r.id}>{r.name} ({r.capacity_percentage}% capacity)</option>)}
            </select>
          </div>
